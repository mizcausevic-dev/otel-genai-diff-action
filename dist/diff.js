 ...
 */
export function diffRollups(previous, next, opts = {}) {
    assertReport(previous, "previous");
    assertReport(next, "next");
    const threshold = opts.threshold ?? 0.1;
    const changes = [];
    const push = (reason, subject, fields = {}) => {
        const e = { reason, subject, ...fields };
        changes.push(e);
    };
    // ─── per-row indexing ─────────────────────────────────────────────────
    const prevByKey = new Map();
    for (const r of previous.rows)
        prevByKey.set(rowKey(r), r);
    const nextByKey = new Map();
    for (const r of next.rows)
        nextByKey.set(rowKey(r), r);
...
function assertReport(r, side) {
    if (!r || typeof r !== "object")
        throw new Error(`${side} must be a RollupReport object`);
    if (!Array.isArray(r.rows))
        throw new Error(`${side}.rows must be an array`);
    if (!r.totals || typeof r.totals !== "object")
        throw new Error(`${side}.totals is required`);
}
