...
export interface RollupDiff {
  changes: DiffEntry[];
  breaking: boolean;
  added: { models: string[]; providers: string[] };
  removed: { models: string[]; providers: string[] };
  totals: {
    previous: { spans: number; inputTokens: number; outputTokens: number; costUSD: number };
    next: { spans: number; inputTokens: number; outputTokens: number; costUSD: number };
  };
}
