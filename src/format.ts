import type { ChangeReason, RollupDiff } from "./types.js";
...
function formatRatio(r: number | undefined): string {
  if (r === undefined || !Number.isFinite(r)) return r === Number.POSITIVE_INFINITY ? "+∞" : "";
  const pct = r * 100;
  return `${pct >= 0 ? "+" : ""}${pct.toFixed(1)}%`;
}

export function toMarkdown(diff: RollupDiff): string {
  if (diff.changes.length === 0) return `**No changes.** Rollups are equivalent within the configured threshold.`;
  const lines: string[] = [];
  lines.push(diff.breaking ? `## OTel GenAI rollup diff (**BREAKING**)` : `## OTel GenAI rollup diff`);
  lines.push(``);
  lines.push(
    `**Totals:** spans ${diff.totals.previous.spans} → ${diff.totals.next.spans} · input tokens ${diff.totals.previous.inputTokens} → ${diff.totals.next.inputTokens} · output tokens ${diff.totals.previous.outputTokens} → ${diff.totals.next.outputTokens} · cost \`$${diff.totals.previous.costUSD.toFixed(4)}\` → \`$${diff.totals.next.costUSD.toFixed(4)}\``
  );
  lines.push(``);
...
export function toSummary(diff: RollupDiff): string {
